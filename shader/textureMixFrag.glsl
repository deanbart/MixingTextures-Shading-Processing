...
void main(void) {
	...
	vec4 col0 = texture2D(sun, p); // color of texture 0
	vec4 col1 = texture2D(moon, p); // color of texture 1
	vec4 col2 = texture2D(earth, p); // color of texture 2
	...
	// do something, depending on the mixMode (0=subtle, 1=regular, 2=obvious)
	if (mixMode==0) {
